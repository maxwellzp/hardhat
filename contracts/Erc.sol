...
contract ERC20 is IERC20 {
    ...
    // Я как иницииатор хочу перевести кому-то еще какое-то количество токенов
    // Перевод с кошелька инициатора транзакции на какой-то другой адресс
    ...
    function transformFrom(address sender, address recipient, uint amount) external enoughTokens(sender, amount) {
...
contract MShop {
    ...
    function sell(uint _amountToSell) external {
        ...
        // allowance - то что я разрешил списывать у себя
        uint allowance = token.allowance(msg.sender, address(this));
        require(allowance >= _amountToSell, "check allowance!");

        token.transformFrom(msg.sender, address(this), _amountToSell);
    ...
    // вызывается автоматически если в контракт пришли деньги
    // всегда - external payable
